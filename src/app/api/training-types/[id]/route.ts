...
export async function PATCH(
  ...
  try {
    ...
    const existing = await prisma.trainingType.findFirst({ where: { id, organizationId: orgId } })
    ...
    await logActivity(orgId, profile.id, profile.email, 'TRAINING_TYPE_UPDATED',
      { type: 'TrainingType', id }, { name: updated.name })

    return NextResponse.json({ ok: true, data: updated })
  } catch (e: unknown) {
    console.error('[PATCH /api/training-types/[id]]', e)
...
export async function DELETE(
  ...
  try {
    ...
    await logActivity(orgId, profile.id, profile.email, 'TRAINING_TYPE_DELETED',
      { type: 'TrainingType', id }, { name: existing.name })

    return NextResponse.json({ ok: true })
  } catch (e) {
    console.error('[DELETE /api/training-types/[id]]', e)
