...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, data: universities })
  } catch (e) {
    console.error('[GET /api/universities]', e)
...
export async function POST(req: NextRequest) {
  try {
    ...
    await logActivity(orgId, profile.id, profile.email, 'UNIVERSITY_CREATED',
      { type: 'University', id: university.id }, { name: university.name })

    return NextResponse.json({ ok: true, data: university }, { status: 201 })
  } catch (e: unknown) {
    console.error('[POST /api/universities]', e)
