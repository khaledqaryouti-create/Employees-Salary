...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, data: deductionTypes })
  } catch (e) {
    console.error('[GET /api/deduction-types]', e)
    return NextResponse.json({ error: 'Failed to load deduction types' }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    await logActivity(orgId, profile.id, profile.email, 'DEDUCTION_TYPE_CREATED',
      { type: 'DeductionType', id: deductionType.id }, { name: deductionType.name })

    return NextResponse.json({ ok: true, data: deductionType }, { status: 201 })
  } catch (e: unknown) {
    console.error('[POST /api/deduction-types]', e)
