...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, data: certificateTypes })
  } catch (e) {
    console.error('[GET /api/certificate-types]', e)
...
export async function POST(req: NextRequest) {
  try {
    ...
    await logActivity(orgId, profile.id, profile.email, 'CERTIFICATE_TYPE_CREATED',
      { type: 'CertificateType', id: certificateType.id }, { name: certificateType.name })

    return NextResponse.json({ ok: true, data: certificateType }, { status: 201 })
  } catch (e: unknown) {
    console.error('[POST /api/certificate-types]', e)
