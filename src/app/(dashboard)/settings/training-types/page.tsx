...
export default function TrainingTypesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/training-types?${params.toString()}`)
      if (res.ok) {
        const json = await res.json() as { ok: boolean; data: TrainingType[] }
        setItems(json.data ?? [])
      }
    } finally {
      setLoading(false)
  ...
  function openEdit(t: TrainingType) {
    setEditId(t.id)
    setForm({
      name:        t.name,
      nameAr:      t.nameAr      ?? '',
  ...
  async function save() {
    ...
    setSaving(true)
    try {
      const url    = editId ? `/api/training-types/${editId}` : '/api/training-types'
      ...
      toast.success(editId ? 'Training type updated' : 'Training type created')
      setDialogOpen(false)
      await loadData()
    } finally {
      setSaving(false)
  ...
  async function toggleActive(item: TrainingType) {
    const res = await fetch(`/api/training-types/${item.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteItem() {
    if (!deleteId) return
    const res = await fetch(`/api/training-types/${deleteId}`, { method: 'DELETE' })
    if (res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Manage training type categories for employee training records
          </p>
        </div>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Training Types List</CardTitle>
          <CardDescription>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {!loading && items.length === 0 && (
                  <tr>
                    <td colSpan={5} className="text-center py-10 text-gray-400">No training types found</td>
                  </tr>
                )}
                {items.map((item) => (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editId ? 'Edit Training Type' : 'Add Training Type'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="tt-name-ar">Arabic Name</Label>
              <Input
                id="tt-name-ar"
                dir="rtl"
                placeholder="مثال: تقني، مهارات ناعمة، امتثال"
                value={form.nameAr}
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-gray-600">
            This training type will be permanently deleted. This action cannot be undone.
          </p>
          <DialogFooter>
