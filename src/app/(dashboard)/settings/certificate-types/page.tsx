...
export default function CertificateTypesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/certificate-types?${params.toString()}`)
      if (res.ok) {
        const json = await res.json() as { ok: boolean; data: CertificateType[] }
        setItems(json.data ?? [])
      }
    } finally {
      setLoading(false)
  ...
  function openEdit(item: CertificateType) {
    setEditId(item.id)
    setForm({
      name:        item.name,
      nameAr:      item.nameAr      ?? '',
  ...
  async function save() {
    ...
    setSaving(true)
    try {
      const url    = editId ? `/api/certificate-types/${editId}` : '/api/certificate-types'
      ...
      toast.success(editId ? 'Certificate type updated' : 'Certificate type created')
      setDialogOpen(false)
      await loadData()
    } finally {
      setSaving(false)
  ...
  async function toggleActive(item: CertificateType) {
    const res = await fetch(`/api/certificate-types/${item.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteItem() {
    if (!deleteId) return
    const res = await fetch(`/api/certificate-types/${deleteId}`, { method: 'DELETE' })
    if (res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Manage certificate type categories for employee certification records
          </p>
        </div>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Certificate Types List</CardTitle>
          <CardDescription>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {!loading && items.length === 0 && (
                  <tr>
                    <td colSpan={5} className="text-center py-10 text-gray-400">No certificate types found</td>
                  </tr>
                )}
                {items.map((item) => (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editId ? 'Edit Certificate Type' : 'Add Certificate Type'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input
                id="cert-name"
                placeholder="e.g. Professional, Academic, Vocational"
                value={form.name}
                onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="cert-name-ar">Arabic Name</Label>
              <Input
                id="cert-name-ar"
                dir="rtl"
                placeholder="مثال: مهني، أكاديمي، مهني"
                value={form.nameAr}
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-gray-600">
            This certificate type will be permanently deleted. This action cannot be undone.
          </p>
          <DialogFooter>
